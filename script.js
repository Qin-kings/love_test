...
const C = {
  ...
  uploaderName: 'uploaderName'  // 新增上传者名称配置
};
...
function loadCfgToForm() {
  ...
  $('#uploaderName').value = localStorage.getItem(C.uploaderName) || '小情侣'; // 新增
...
function saveFormToCfg() {
  ...
  localStorage.setItem(C.uploaderName, $('#uploaderName').value.trim() || '小情侣'); // 新增
...
function getCfg() {
  return {
    ...
    uploaderName: $('#uploaderName').value.trim() || localStorage.getItem(C.uploaderName) || '小情侣' // 新增
  };
}
...
async function renderGallery(page = 1) {
  try {
    ...
    currentPageData.forEach((item, idx) => {
      ...
      div.setAttribute('data-uploader', item.uploader || '未知');
      div.setAttribute('data-time', item.time || '未知');
      div.setAttribute('data-name', item.name || '未知');
...
function addGalleryClickHandlers() {
  ...
  gallery.addEventListener('click', function(e) {
    ...
    if (item) {
      ...
      if (src) {
        openImageModal(src, uploader, time, name);
      }
    }
  });
}
...
async function startUpload() {
  ...
  try {
    ...
    const addedUrls = [];
    const { uploaderName } = getCfg(); // 获取上传者名称
    ...
    for (const f of files) {
      ...
      const item = {
        ...
        who: '我们',
        ts: Date.now(),
        uploader: uploaderName, // 保存上传者
        time: new Date().toLocaleString('zh-CN'), // 保存上传时间
        name: f.name // 保存文件名
      };
      list.push(item);
      addedUrls.push(url);
...
// 图片放大功能
function openImageModal(src, uploader, time, name) {
  ...
  // 设置图片信息
  document.getElementById('infoUploader').textContent = uploader || '未知';
  document.getElementById('infoTime').textContent = time || '未知';
  document.getElementById('infoName').textContent = name || '未知';
